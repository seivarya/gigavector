 ...
 *
 * Pass to inference_create() after initializing with
 * inference_config_init().  All string pointers must remain valid
 * until inference_create() returns (they are copied internally).
 */
typedef struct {
 ...
 *
 * The database must already be open (via db_open or equivalent) and
 ...
 */
GV_InferenceEngine *inference_create(void *db, const GV_InferenceConfig *config);
 ...
 *
 * The filter expression follows the same syntax as filter_parse().
 *
 * @param eng         Engine handle; must be non-NULL.
 * @param query_text  Query text to embed; must be non-NULL.
 * @param k           Maximum number of results to return.
 ...
 * (equivalent to inference_add).  Otherwise the existing vector and
