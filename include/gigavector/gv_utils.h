...
static inline int gv_write_u64(FILE *f, uint64_t v) {
    return fwrite(&v, sizeof(uint64_t), 1, f) == 1 ? 0 : -1;
}

static inline int gv_read_u64(FILE *f, uint64_t *v) {
    return (v && fread(v, sizeof(uint64_t), 1, f) == 1) ? 0 : -1;
}

static inline int gv_write_f32(FILE *f, float v) {
    return fwrite(&v, sizeof(float), 1, f) == 1 ? 0 : -1;
}

static inline int gv_read_f32(FILE *f, float *v) {
    return (v && fread(v, sizeof(float), 1, f) == 1) ? 0 : -1;
}

static inline int gv_write_floats(FILE *f, const float *data, size_t count) {
    return fwrite(data, sizeof(float), count, f) == count ? 0 : -1;
}

static inline int gv_read_floats(FILE *f, float *data, size_t count) {
    return (data && fread(data, sizeof(float), count, f) == count) ? 0 : -1;
}

static inline int gv_write_bytes(FILE *f, const void *data, size_t count) {
    return fwrite(data, 1, count, f) == count ? 0 : -1;
}

static inline int gv_read_bytes(FILE *f, void *data, size_t count) {
    return (data && fread(data, 1, count, f) == count) ? 0 : -1;
}

static inline int gv_write_size(FILE *f, size_t v) {
    return fwrite(&v, sizeof(size_t), 1, f) == 1 ? 0 : -1;
}

static inline int gv_read_size(FILE *f, size_t *v) {
    return (v && fread(v, sizeof(size_t), 1, f) == 1) ? 0 : -1;
}

/* Write a string with automatic strlen. */
static inline int gv_write_string(FILE *f, const char *s) {
...
static inline char *gv_read_string(FILE *f) {
    uint32_t len;
    if (gv_read_u32(f, &len) != 0) return NULL;
...
static inline int gv_write_metadata(FILE *f, const GV_Metadata *meta) {
    uint32_t count = 0;
    for (const GV_Metadata *c = meta; c; c = c->next) count++;
    if (gv_write_u32(f, count) != 0) return -1;
    for (const GV_Metadata *c = meta; c; c = c->next) {
        if (gv_write_string(f, c->key) != 0) return -1;
        if (gv_write_string(f, c->value) != 0) return -1;
    }
    return 0;
}

static inline float gv_cosine_similarity(const float *a, const float *b, size_t dim) {
...
static inline const char *gv_metadata_get_direct(GV_Metadata *metadata, const char *key) {
...
static inline GV_Metadata *gv_metadata_copy(GV_Metadata *src) {
