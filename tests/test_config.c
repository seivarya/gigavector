...
static int test_cpu_has_feature(void) {
    int has_sse = gv_cpu_has_feature(GV_CPU_FEATURE_SSE);
    int has_sse2 = gv_cpu_has_feature(GV_CPU_FEATURE_SSE2);
    int has_sse3 = gv_cpu_has_feature(GV_CPU_FEATURE_SSE3);
    int has_sse4_1 = gv_cpu_has_feature(GV_CPU_FEATURE_SSE4_1);
    int has_sse4_2 = gv_cpu_has_feature(GV_CPU_FEATURE_SSE4_2);
    int has_avx = gv_cpu_has_feature(GV_CPU_FEATURE_AVX);
    int has_avx2 = gv_cpu_has_feature(GV_CPU_FEATURE_AVX2);
    int has_avx512f = gv_cpu_has_feature(GV_CPU_FEATURE_AVX512F);
    int has_fma = gv_cpu_has_feature(GV_CPU_FEATURE_FMA);
...
static int test_cpu_has_feature_none(void) {
    int has_none = gv_cpu_has_feature(GV_CPU_FEATURE_NONE);
    ASSERT(has_none == 0 || has_none != 0, "NONE feature check works");
    return 0;
}

static int test_cpu_caching(void) {
    unsigned int f1 = gv_cpu_detect_features();
    unsigned int f2 = gv_cpu_detect_features();
