...
static int test_create_destroy(void) {
    ...
    GV_VacuumManager *mgr = vacuum_create(db, &config);
    ASSERT(mgr != NULL, "vacuum_create returned NULL");
    ASSERT(vacuum_get_state(mgr) == GV_VACUUM_IDLE, "initial state should be IDLE");
...
static int test_manual_vacuum(void) {
    ...
    db_delete_vector_by_index(db, 1);
    db_delete_vector_by_index(db, 3);
...
static int test_get_fragmentation(void) {
    ...
    double frag = vacuum_get_fragmentation(mgr);
    ASSERT(frag >= 0.0, "fragmentation should be >= 0");

    db_delete_vector_by_index(db, 0);
    double frag2 = vacuum_get_fragmentation(mgr);
...
static int test_get_stats(void) {
    ...
    int rc = vacuum_get_stats(mgr, &stats);
...
static int test_auto_vacuum_start_stop(void) {
    ...
    int rc = vacuum_start_auto(mgr);
    ASSERT(rc == 0, "start_auto should succeed");

    rc = vacuum_stop_auto(mgr);
...
static int test_vacuum_with_null_config(void) {
    ...
    GV_VacuumManager *mgr = vacuum_create(db, NULL);
