...
static int test_sparse_basic_insert_search(void) {
    GV_Database *db = db_open(NULL, 100, GV_INDEX_TYPE_SPARSE);
    if (db == NULL) {
        return 0;
    }
    ...
    if (db_add_sparse_vector(db, indices1, values1, 3, 100, NULL, NULL) != 0) {
        db_close(db);
        return 0;
    }
    ...
    if (db_add_sparse_vector(db, indices2, values2, 3, 100, NULL, NULL) != 0) {
        db_close(db);
        return 0;
    }
    ...
    int n = db_search_sparse(db, q_indices, q_values, 2, 2, res, GV_DISTANCE_DOT_PRODUCT);
    if (n < 0) {
        db_close(db);
        return 0;
    }
...
static int test_sparse_cosine_distance(void) {
    ...
    ASSERT(db_add_sparse_vector(db, indices, values, 3, 100, NULL, NULL) == 0, "add sparse vector");
    
    uint32_t q_indices[3] = {0, 10, 50};
    float q_values[3] = {1.0f, 2.0f, 3.0f};
    GV_SearchResult res[1];
    int n = db_search_sparse(db, q_indices, q_values, 3, 1, res, GV_DISTANCE_COSINE);
...
static int test_sparse_metadata(void) {
    ...
    uint32_t indices[3] = {0, 10, 50};
    float values[3] = {1.0f, 2.0f, 3.0f};
    if (db_add_sparse_vector(db, indices, values, 3, 100, "category", "test") != 0) {
        db_close(db);
        return 0;
    }
    ...
    float q_values[3] = {1.0f, 2.0f, 3.0f};
    GV_SearchResult res[1];
    int n = db_search_sparse(db, q_indices, q_values, 3, 1, res, GV_DISTANCE_DOT_PRODUCT);
...
static int test_sparse_large_dataset(void) {
    GV_Database *db = db_open(NULL, 1000, GV_INDEX_TYPE_SPARSE);
    if (db == NULL) {
        return 0;
    }
    ...
    for (int i = 0; i < 50; i++) {
        ...
        if (db_add_sparse_vector(db, indices, values, 5, 1000, NULL, NULL) != 0) {
            db_close(db);
            return 0;
        }
    }
    ...
    int n = db_search_sparse(db, q_indices, q_values, 3, 5, res, GV_DISTANCE_DOT_PRODUCT);
...
static int test_sparse_empty_query(void) {
    ...
    uint32_t indices[3] = {0, 10, 50};
    float values[3] = {1.0f, 2.0f, 3.0f};
    if (db_add_sparse_vector(db, indices, values, 3, 100, NULL, NULL) != 0) {
        db_close(db);
        return 0;
    }
    ...
    int n = db_search_sparse(db, q_indices, q_values, 0, 1, res, GV_DISTANCE_DOT_PRODUCT);
    if (n < 0) {
        db_close(db);
        return 0;
    }
...
static int test_sparse_persistence(void) {
    ...
    GV_Database *db = db_open(path, 100, GV_INDEX_TYPE_SPARSE);
    if (db == NULL) {
        return 0;
    }
    ...
    GV_Database *db2 = db_open(path, 100, GV_INDEX_TYPE_SPARSE);
    if (db2 == NULL) {
        remove(path);
        return 0;
    }
    ...
    float q_values[3] = {1.0f, 2.0f, 3.0f};
    GV_SearchResult res[1];
    int n = db_search_sparse(db2, q_indices, q_values, 3, 1, res, GV_DISTANCE_DOT_PRODUCT);
