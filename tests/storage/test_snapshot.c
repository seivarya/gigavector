...
static int test_manager_create_destroy(void) {
    GV_SnapshotManager *mgr = snapshot_manager_create(10);
    ASSERT(mgr != NULL, "snapshot_manager_create returned NULL");
...
static int test_create_and_open_snapshot(void) {
    ...
    float vectors[8] = {1.0f, 2.0f, 3.0f, 4.0f,
                         5.0f, 6.0f, 7.0f, 8.0f};
    uint64_t sid = snapshot_create(mgr, 2, vectors, 4, "test-snap-1");
    ASSERT(sid > 0, "snapshot_create should return nonzero id");

    GV_Snapshot *snap = snapshot_open(mgr, sid);
    ASSERT(snap != NULL, "snapshot_open should return non-NULL");
    ASSERT(snapshot_count(snap) == 2, "snapshot count should be 2");
    ASSERT(snapshot_dimension(snap) == 4, "snapshot dimension should be 4");
...
static int test_snapshot_list(void) {
    ...
    snapshot_create(mgr, 1, v1, 4, "snap-a");
    snapshot_create(mgr, 1, v2, 4, "snap-b");

    GV_SnapshotInfo infos[10];
    int count = snapshot_list(mgr, infos, 10);
...
static int test_snapshot_delete(void) {
    ...
    uint64_t sid = snapshot_create(mgr, 1, v, 4, "to-delete");
    ASSERT(sid > 0, "create snapshot");

    int rc = snapshot_delete(mgr, sid);
...
static int test_snapshot_empty(void) {
    ...
    uint64_t sid = snapshot_create(mgr, 0, NULL, 4, "empty");
    ASSERT(sid > 0, "empty snapshot should get valid id");

    GV_Snapshot *snap = snapshot_open(mgr, sid);
    ASSERT(snap != NULL, "open empty snapshot");
    ASSERT(snapshot_count(snap) == 0, "empty snapshot count == 0");
