...
static int test_create_api_key_config(void) {
    ...
    GV_AuthManager *mgr = auth_create(&cfg);
...
static int test_generate_verify_api_key(void) {
    ...
    int rc = auth_generate_api_key(mgr, "test key", 0, key_out, key_id_out);
    ...
    GV_AuthResult result = auth_verify_api_key(mgr, key_out, &identity);
...
static int test_revoke_api_key(void) {
    ...
    char key_out[128] = {0};
    char key_id_out[64] = {0};
    int rc = auth_generate_api_key(mgr, "revoke-test", 0, key_out, key_id_out);
    ASSERT(rc == 0, "generate API key for revocation");

    rc = auth_revoke_api_key(mgr, key_id_out);
...
static int test_list_api_keys(void) {
    ...
    auth_generate_api_key(mgr, "key-a", 0, key_out, key_id_out);
    auth_generate_api_key(mgr, "key-b", 0, key_out, key_id_out);

    GV_APIKey *keys = NULL;
    size_t count = 0;
    int rc = auth_list_api_keys(mgr, &keys, &count);
    ASSERT(rc == 0, "list API keys");
    ASSERT(count >= 2, "should have at least 2 keys");

    auth_free_api_keys(keys, count);
    auth_destroy(mgr);
    return 0;
}
...
static int test_sha256_and_hex(void) {
    ...
    int rc = auth_sha256(data, strlen(data), hash);
    ASSERT(rc == 0, "sha256 computation");

    char hex[65];
    auth_to_hex(hash, 32, hex);
