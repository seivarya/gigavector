...
static int test_create_destroy(void) {
    GV_TTLManager *mgr = ttl_create(NULL);
    ASSERT(mgr != NULL, "ttl_create(NULL) should succeed");
    ttl_destroy(mgr);
    ...
    mgr = ttl_create(&cfg);
    ASSERT(mgr != NULL, "ttl_create with config should succeed");
    ttl_destroy(mgr);
...
static int test_set_and_get(void) {
    ...
    ASSERT(ttl_set(mgr, 5, 3600) == 0, "set ttl");

    uint64_t expire_at = 0;
    ASSERT(ttl_get(mgr, 5, &expire_at) == 0, "get ttl");
    ASSERT(expire_at > 0, "expire_at should be non-zero after set");

    uint64_t no_ttl = 999;
    ASSERT(ttl_get(mgr, 99, &no_ttl) == 0, "get ttl on unset index");
...
static int test_set_absolute(void) {
    ...
    ASSERT(ttl_set_absolute(mgr, 0, future) == 0, "set_absolute");

    uint64_t expire_at = 0;
    ASSERT(ttl_get(mgr, 0, &expire_at) == 0, "get after set_absolute");
    ASSERT(expire_at == future, "expire_at should match the absolute time");

    ASSERT(ttl_set_absolute(mgr, 0, 0) == 0, "set_absolute 0 to remove");
    ASSERT(ttl_get(mgr, 0, &expire_at) == 0, "get after remove");
...
static int test_remove(void) {
    ...
    ASSERT(ttl_set(mgr, 10, 600) == 0, "set ttl");
    ASSERT(ttl_remove(mgr, 10) == 0,    "remove ttl");

    uint64_t expire_at = 999;
    ASSERT(ttl_get(mgr, 10, &expire_at) == 0, "get after remove");
...
static int test_is_expired(void) {
    ...
    ASSERT(ttl_set_absolute(mgr, 1, 1) == 0, "set to past timestamp");
    int expired = ttl_is_expired(mgr, 1);
    ASSERT(expired == 1, "vector with past timestamp should be expired");

    /* Set expiration far in the future */
    ASSERT(ttl_set_absolute(mgr, 2, 9999999999ULL) == 0, "set to future timestamp");
    expired = ttl_is_expired(mgr, 2);
    ASSERT(expired == 0, "vector with future timestamp should not be expired");

    expired = ttl_is_expired(mgr, 42);
...
static int test_get_remaining(void) {
    ...
    ASSERT(ttl_set(mgr, 0, 7200) == 0, "set 2h ttl");
    uint64_t rem = 0;
    ASSERT(ttl_get_remaining(mgr, 0, &rem) == 0, "get_remaining");
    ASSERT(rem > 0, "remaining should be positive for future TTL");

    /* Set TTL in the past */
    ASSERT(ttl_set_absolute(mgr, 1, 1) == 0, "set past");
    ASSERT(ttl_get_remaining(mgr, 1, &rem) == 0, "get_remaining past");
...
static int test_bulk_and_stats(void) {
    ...
    int set = ttl_set_bulk(mgr, indices, 5, 1800);
    ASSERT(set == 5, "set_bulk should return 5");

    GV_TTLStats stats;
    memset(&stats, 0, sizeof(stats));
    ASSERT(ttl_get_stats(mgr, &stats) == 0, "get_stats");
