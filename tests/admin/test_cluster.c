...
static int test_create_destroy(void) {
    ...
    GV_Cluster *cluster = cluster_create(&cfg);
    ASSERT(cluster != NULL, "cluster_create should succeed");
...
static int test_get_local_node(void) {
    ...
    int rc = cluster_get_local_node(cluster, &info);
    ASSERT(rc == 0, "cluster_get_local_node should succeed");
...
static int test_get_stats(void) {
    ...
    int rc = cluster_get_stats(cluster, &stats);
    ASSERT(rc == 0, "cluster_get_stats should succeed");
...
static int test_is_healthy(void) {
    ...
    int healthy = cluster_is_healthy(cluster);
...
static int test_list_nodes(void) {
    ...
    size_t count = 0;
    int rc = cluster_list_nodes(cluster, &nodes, &count);
    ASSERT(rc == 0, "cluster_list_nodes should succeed");
...
static int test_get_shard_manager(void) {
    ...
    GV_ShardManager *sm = cluster_get_shard_manager(cluster);
    ASSERT(sm != NULL, "cluster_get_shard_manager should return non-NULL");
...
static int test_start_stop(void) {
    ...
    int rc = cluster_start(cluster);
    ASSERT(rc == 0, "cluster_start should succeed");

    rc = cluster_stop(cluster);
    ASSERT(rc == 0, "cluster_stop should succeed");
...
static int test_get_node_by_id(void) {
    ...
    int rc = cluster_get_node(cluster, "lookup-node", &info);
    ASSERT(rc == 0, "cluster_get_node for local node should succeed");

    GV_NodeInfo info2;
    memset(&info2, 0, sizeof(info2));
    rc = cluster_get_node(cluster, "nonexistent-node-xyz", &info2);
    ASSERT(rc != 0, "cluster_get_node for unknown node should fail");
