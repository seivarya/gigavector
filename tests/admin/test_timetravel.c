...
static int test_create_destroy(void) {
    GV_TimeTravelManager *mgr = tt_create(NULL);
    ASSERT(mgr != NULL, "tt_create(NULL) should succeed");
    tt_destroy(mgr);
    ...
    mgr = tt_create(&cfg);
    ASSERT(mgr != NULL, "tt_create with config should succeed");
    tt_destroy(mgr);
...
static int test_record_update(void) {
    ...
    uint64_t v1 = tt_record_insert(mgr, 0, old_vec, 2);
    ASSERT(v1 > 0, "insert");

    uint64_t v2 = tt_record_update(mgr, 0, old_vec, new_vec, 2);
...
static int test_record_delete(void) {
    ...
    float vec[] = {3.0f, 4.0f};
    uint64_t v1 = tt_record_insert(mgr, 0, vec, 2);
    ASSERT(v1 > 0, "insert");

    uint64_t v2 = tt_record_delete(mgr, 0, vec, 2);
...
static int test_query_at_version(void) {
    ...
    uint64_t v1 = tt_record_insert(mgr, 0, vec1, 2);
    ASSERT(v1 > 0, "insert v1");

    float vec2[] = {10.0f, 20.0f};
    uint64_t v2 = tt_record_update(mgr, 0, vec1, vec2, 2);
    ASSERT(v2 > 0, "update to v2");

    float out[2] = {0};
    int found = tt_query_at_version(mgr, v2, 0, out, 2);
    ASSERT(found == 1, "should find vector at v2");
    ASSERT(out[0] == 10.0f && out[1] == 20.0f, "data at v2 should be updated values");

    float out_old[2] = {0};
    found = tt_query_at_version(mgr, v1, 0, out_old, 2);
...
static int test_list_versions(void) {
    ...
    float vec[] = {0.5f};
    tt_record_insert(mgr, 0, vec, 1);
    tt_record_insert(mgr, 1, vec, 1);
    tt_record_insert(mgr, 2, vec, 1);

    GV_VersionEntry entries[10];
    memset(entries, 0, sizeof(entries));
    int count = tt_list_versions(mgr, entries, 10);
...
static int test_gc(void) {
    ...
    GV_TimeTravelManager *mgr = tt_create(&cfg);
    ASSERT(mgr != NULL, "create");

    float vec[] = {1.0f};
    for (int i = 0; i < 10; i++) {
        uint64_t v = tt_record_insert(mgr, (size_t)i, vec, 1);
        ASSERT(v > 0, "insert");
    }

    int removed = tt_gc(mgr);
    ...
    int remaining = tt_list_versions(mgr, entries, 20);
...
static int test_save_load(void) {
    ...
    float vec[] = {1.0f, 2.0f};
    tt_record_insert(mgr, 0, vec, 2);
    tt_record_insert(mgr, 1, vec, 2);
    uint64_t saved_version = tt_current_version(mgr);

    ASSERT(tt_save(mgr, path) == 0, "save");
    tt_destroy(mgr);

    GV_TimeTravelManager *loaded = tt_load(path);
    ASSERT(loaded != NULL, "load should succeed");
    ASSERT(tt_current_version(loaded) == saved_version,
           "loaded current_version should match saved");

    GV_VersionEntry entries[10];
    int count = tt_list_versions(loaded, entries, 10);
