...
static GV_BM25Index *make_bm25(void) {
    ...
    if (!bm) return NULL;
    bm25_add_document(bm, 0, "alpha bravo charlie");
    bm25_add_document(bm, 1, "delta echo foxtrot");
    bm25_add_document(bm, 2, "alpha delta gamma");
    return bm;
}

static int test_config_defaults(void) {
    GV_HybridConfig cfg;
    hybrid_config_init(&cfg);
...
static int test_create_destroy(void) {
    ...
    GV_HybridSearcher *hs = hybrid_create(db, bm, NULL);
    ASSERT(hs != NULL, "hybrid_create should succeed");
...
static int test_linear_fusion_util(void) {
    double score = hybrid_linear_fusion(0.8, 0.6, 0.7, 0.3);
...
static int test_rrf_fusion_util(void) {
    ...
    double score = hybrid_rrf_fusion(1, 2, 60.0);
    ...
    double score2 = hybrid_rrf_fusion(3, 0, 60.0);
...
static int test_normalize_score(void) {
    double n = hybrid_normalize_score(5.0, 2.0, 10.0);
    /* (5-2)/(10-2) = 3/8 = 0.375 */
    ASSERT(fabs(n - 0.375) < 0.001, "normalize should map to [0,1]");

    double n2 = hybrid_normalize_score(5.0, 5.0, 5.0);
...
static int test_set_weights(void) {
    ...
    int rc = hybrid_set_weights(hs, 0.8, 0.2);
    ASSERT(rc == 0, "set_weights should succeed");

    GV_HybridConfig cfg;
    rc = hybrid_get_config(hs, &cfg);
...
static int test_hybrid_search_basic(void) {
    ...
    int n = hybrid_search(hs, query, "alpha", 3, results);
...
static int test_set_config(void) {
    ...
    cfg.rrf_k = 30.0;
    int rc = hybrid_set_config(hs, &cfg);
    ASSERT(rc == 0, "set_config should succeed");

    GV_HybridConfig out;
    rc = hybrid_get_config(hs, &out);
    ASSERT(rc == 0, "get_config should succeed");
