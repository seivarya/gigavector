...
static int test_threshold_passes_euclidean(void) {
    ...
    ASSERT(threshold_passes(1.0f, 1.0f, GV_DISTANCE_EUCLIDEAN) == 1,
           "1.0 == 1.0 should pass for euclidean (boundary)");
    return 0;
}

static int test_threshold_passes_manhattan(void) {
    ASSERT(threshold_passes(0.3f, 0.5f, GV_DISTANCE_MANHATTAN) == 1,
           "0.3 <= 0.5 should pass for manhattan");
    ASSERT(threshold_passes(0.8f, 0.5f, GV_DISTANCE_MANHATTAN) == 0,
...
static int test_threshold_filter_basic(void) {
    ...
    size_t count = threshold_filter(results, 5, 1.0f, GV_DISTANCE_EUCLIDEAN);
...
static int test_threshold_filter_none_pass(void) {
    ...
    size_t count = threshold_filter(results, 2, 0.1f, GV_DISTANCE_EUCLIDEAN);
...
static int test_threshold_filter_all_pass(void) {
    ...
    size_t count = threshold_filter(results, 3, 100.0f, GV_DISTANCE_EUCLIDEAN);
...
static int test_search_with_threshold(void) {
    ...
    int n = db_search_with_threshold(db, query, 4, GV_DISTANCE_EUCLIDEAN,
                                         0.5f, results);
...
static int test_threshold_filter_empty(void) {
    size_t count = threshold_filter(NULL, 0, 1.0f, GV_DISTANCE_EUCLIDEAN);
