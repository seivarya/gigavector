...
static int test_whitespace_tokenizer(void) {
    ...
    GV_Tokenizer *tok = tokenizer_create(&cfg);
...
static int test_token_positions(void) {
    ...
    GV_TokenList list = {0};
    tokenizer_tokenize(tok, text, strlen(text), &list);
...
static int test_tokenize_simple(void) {
    GV_TokenList list = {0};
    int rc = tokenize_simple("hello world foo", &list);
...
static int test_unique_tokens(void) {
    ...
    char **unique = NULL;
    size_t ucount = 0;
    int rc = token_list_unique(&list, &unique, &ucount);
    ASSERT(rc == 0, "unique should succeed");
    ASSERT(ucount == 3, "3 unique tokens");
    unique_tokens_free(unique, ucount);
...
static int test_is_stopword(void) {
    ASSERT(is_stopword("the") == 1, "'the' is a stopword");
    ASSERT(is_stopword("and") == 1, "'and' is a stopword");
    ASSERT(is_stopword("elephant") == 0, "'elephant' is not a stopword");
    ASSERT(is_stopword("") == 0, "empty is not a stopword");
    return 0;
}
