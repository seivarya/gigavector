...
static int test_payload_index_create_destroy(void) {
    GV_PayloadIndex *idx = payload_index_create();
    ASSERT(idx != NULL, "payload index creation should succeed");

    payload_index_destroy(idx);

    payload_index_destroy(NULL);
    return 0;
}
...
static int test_payload_index_add_remove_field(void) {
    ...
    int rc = payload_index_add_field(idx, "age", GV_FIELD_INT);
    ASSERT(rc == 0, "adding int field should succeed");

    rc = payload_index_add_field(idx, "name", GV_FIELD_STRING);
    ASSERT(rc == 0, "adding string field should succeed");

    ASSERT(payload_index_field_count(idx) == 2, "should have 2 fields");

    rc = payload_index_remove_field(idx, "age");
    ASSERT(rc == 0, "removing field should succeed");
    ASSERT(payload_index_field_count(idx) == 1, "should have 1 field after removal");
...
static int test_payload_index_insert_int(void) {
    ...
    payload_index_add_field(idx, "score", GV_FIELD_INT);

    int rc = payload_index_insert_int(idx, 0, "score", 100);
    ASSERT(rc == 0, "inserting int value for vector 0 should succeed");

    rc = payload_index_insert_int(idx, 1, "score", 200);
    ASSERT(rc == 0, "inserting int value for vector 1 should succeed");

    rc = payload_index_insert_int(idx, 2, "score", 50);
    ASSERT(rc == 0, "inserting int value for vector 2 should succeed");

    ASSERT(payload_index_total_entries(idx) == 3, "should have 3 entries");
...
static int test_payload_index_query_eq(void) {
    ...
    payload_index_add_field(idx, "category", GV_FIELD_STRING);

    payload_index_insert_string(idx, 0, "category", "sports");
    payload_index_insert_string(idx, 1, "category", "tech");
    payload_index_insert_string(idx, 2, "category", "sports");
    payload_index_insert_string(idx, 3, "category", "music");
    ...
    size_t results[10];
    int count = payload_index_query(idx, &query, results, 10);
...
static int test_payload_index_query_range(void) {
    ...
    payload_index_add_field(idx, "price", GV_FIELD_FLOAT);

    payload_index_insert_float(idx, 0, "price", 9.99);
    payload_index_insert_float(idx, 1, "price", 29.99);
    payload_index_insert_float(idx, 2, "price", 49.99);
    payload_index_insert_float(idx, 3, "price", 99.99);
...
static int test_payload_index_query_multi(void) {
    ...
    payload_index_add_field(idx, "age", GV_FIELD_INT);
    payload_index_add_field(idx, "active", GV_FIELD_BOOL);

    payload_index_insert_int(idx, 0, "age", 25);
    payload_index_insert_bool(idx, 0, "active", 1);

    payload_index_insert_int(idx, 1, "age", 30);
    payload_index_insert_bool(idx, 1, "active", 0);

    payload_index_insert_int(idx, 2, "age", 22);
    payload_index_insert_bool(idx, 2, "active", 1);
    ...
    size_t results[10];
    int count = payload_index_query_multi(idx, queries, 2, results, 10);
...
static int test_payload_index_remove_entry(void) {
    ...
    payload_index_add_field(idx, "tag", GV_FIELD_STRING);

    payload_index_insert_string(idx, 0, "tag", "alpha");
    payload_index_insert_string(idx, 1, "tag", "beta");
    ASSERT(payload_index_total_entries(idx) == 2, "should have 2 entries");

    int rc = payload_index_remove(idx, 0);
    ASSERT(rc == 0, "removing vector 0 should succeed");
    ASSERT(payload_index_total_entries(idx) == 1, "should have 1 entry after removal");
