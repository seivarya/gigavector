...
static int test_config_init(void) {
    ...
    late_interaction_config_init(&config);
...
static int test_create_destroy(void) {
    ...
    GV_LateInteractionIndex *idx = late_interaction_create(&config);
    ASSERT(idx != NULL, "late_interaction_create should return non-NULL");
    ASSERT(late_interaction_count(idx) == 0, "new index should have count 0");

    late_interaction_destroy(idx);
    late_interaction_destroy(NULL);
    return 0;
}
...
static int test_add_doc_count(void) {
    ...
    int rc = late_interaction_add_doc(idx, doc0_tokens, 3);
    ASSERT(rc == 0, "add_doc for document 0 should succeed");
    ASSERT(late_interaction_count(idx) == 1, "count should be 1 after one add");

    float doc1_tokens[] = {
        0.5f, 0.5f, 0.0f, 0.0f,
        0.0f, 0.5f, 0.5f, 0.0f
    };
    rc = late_interaction_add_doc(idx, doc1_tokens, 2);
    ASSERT(rc == 0, "add_doc for document 1 should succeed");
    ASSERT(late_interaction_count(idx) == 2, "count should be 2 after two adds");
...
static int test_search(void) {
    ...
    late_interaction_add_doc(idx, doc0, 2);
    late_interaction_add_doc(idx, doc1, 2);

    float query[] = { 0.9f, 0.1f, 0.0f, 0.0f,  0.1f, 0.9f, 0.0f, 0.0f };

    GV_LateInteractionResult results[2];
    int n = late_interaction_search(idx, query, 2, 2, results);
...
static int test_delete(void) {
    ...
    late_interaction_add_doc(idx, doc0, 1);
    late_interaction_add_doc(idx, doc1, 1);
    ASSERT(late_interaction_count(idx) == 2, "count should be 2 before delete");

    int rc = late_interaction_delete(idx, 0);
    ASSERT(rc == 0, "delete doc 0 should succeed");
    ASSERT(late_interaction_count(idx) == 1, "count should be 1 after delete");
...
static int test_stats(void) {
    ...
    int rc = late_interaction_get_stats(idx, &stats);
...
static int test_search_empty(void) {
    ...
    int n = late_interaction_search(idx, query, 1, 5, results);
