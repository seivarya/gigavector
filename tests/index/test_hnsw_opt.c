...
static int test_hnsw_inline_create_destroy(void) {
    ...
    GV_HNSWInlineIndex *idx = hnsw_inline_create(DIM, MAX_ELEMENTS,
                                                      M_PARAM, EF_CONSTRUCT, &config);
    ASSERT(idx != NULL, "hnsw_inline_create returned NULL");
...
static int test_hnsw_inline_create_defaults(void) {
    ...
    ASSERT(hnsw_inline_count(idx) == 0, "empty index should have count 0");
...
static int test_hnsw_inline_insert_count(void) {
    ...
    for (size_t i = 0; i < INSERT_COUNT; i++) {
        ...
        int rc = hnsw_inline_insert(idx, vec, i);
        ASSERT(rc == 0, "insert failed");
    }

    ASSERT(hnsw_inline_count(idx) == INSERT_COUNT,
...
static int test_hnsw_inline_search(void) {
    ...
    for (size_t i = 0; i < INSERT_COUNT; i++) {
        float vec[DIM];
        fill_vector(vec, DIM, (float)i);
        ASSERT(hnsw_inline_insert(idx, vec, i) == 0, "insert failed");
    }
    ...
    size_t labels[5];
    float distances[5];
    int found = hnsw_inline_search(idx, query, 5, 32, labels, distances);
...
static int test_hnsw_inline_search_ordering(void) {
    ...
    size_t labels[10];
    float distances[10];
    int found = hnsw_inline_search(idx, query, 10, 64, labels, distances);
...
static int test_hnsw_inline_rebuild(void) {
    ...
    int rc = hnsw_inline_rebuild(idx, &rconfig);
    ASSERT(rc == 0, "rebuild failed");

    GV_HNSWRebuildStats stats;
    rc = hnsw_inline_rebuild_status(idx, &stats);
...
static int test_hnsw_inline_4bit_quant(void) {
    ...
    size_t labels[3];
    float distances[3];
    int found = hnsw_inline_search(idx, query, 3, 32, labels, distances);
