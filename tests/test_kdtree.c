...
static int test_kdtree_insert_null(void) {
    ...
    int rc = gv_kdtree_insert(NULL, storage, 0, 0);
...
static int test_kdtree_insert_oob(void) {
    ...
    gv_soa_storage_add(storage, data, NULL);
    GV_KDNode *root = NULL;
    int rc = gv_kdtree_insert(&root, storage, 10, 0);
...
static int test_kdtree_insert_basic(void) {
    ...
    float d1[4] = {1,2,3,4};
    float d2[4] = {4,3,2,1};
    gv_soa_storage_add(storage, d1, NULL);
    gv_soa_storage_add(storage, d2, NULL);
    GV_KDNode *root = NULL;
    int rc = gv_kdtree_insert(&root, storage, 0, 0);
    ASSERT(rc == 0, "insert first");
    gv_kdtree_destroy_recursive(root);
    gv_soa_storage_destroy(storage);
    return 0;
}

static int test_kdtree_destroy_null(void) {
    gv_kdtree_destroy_recursive(NULL);
    return 0;
}

static int test_kdtree_destroy_recursive(void) {
    GV_SoAStorage *storage = gv_soa_storage_create(2, 0);
    float d1[2] = {1,2};
    float d2[2] = {3,4};
    gv_soa_storage_add(storage, d1, NULL);
    gv_soa_storage_add(storage, d2, NULL);
    GV_KDNode *root = NULL;
    gv_kdtree_insert(&root, storage, 0, 0);
    gv_kdtree_insert(&root, storage, 1, 0);
    gv_kdtree_destroy_recursive(root);
    gv_soa_storage_destroy(storage);
    return 0;
}
