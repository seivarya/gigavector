...
static int test_create_destroy(void) {
    GV_TieredManager *mgr = tiered_create(NULL);
    ASSERT(mgr != NULL, "tiered_create(NULL) should succeed");
    tiered_destroy(mgr);

    GV_TieredTenantConfig cfg;
    tiered_config_init(&cfg);
    mgr = tiered_create(&cfg);
    ASSERT(mgr != NULL, "tiered_create with config should succeed");
    tiered_destroy(mgr);
...
static int test_add_and_get_info(void) {
    ...
    ASSERT(tiered_add_tenant(mgr, "small_co", GV_TIER_SHARED) == 0, "add tenant");

    GV_TenantInfo info;
    memset(&info, 0, sizeof(info));
    ASSERT(tiered_get_info(mgr, "small_co", &info) == 0, "get_info");
    ASSERT(info.tier == GV_TIER_SHARED, "tier should be SHARED");
    ASSERT(info.vector_count == 0, "initial vector count should be 0");

    ASSERT(tiered_get_info(mgr, "ghost", &info) == -1, "get_info for unknown should fail");
...
static int test_remove_tenant(void) {
    ...
    ASSERT(tiered_add_tenant(mgr, "rm_me", GV_TIER_SHARED) == 0, "add");
    ASSERT(tiered_tenant_count(mgr) == 1, "count should be 1");

    ASSERT(tiered_remove_tenant(mgr, "rm_me") == 0, "remove");
    ASSERT(tiered_tenant_count(mgr) == 0, "count should be 0 after remove");

    ASSERT(tiered_remove_tenant(mgr, "rm_me") == -1, "double remove should fail");
...
static int test_promote(void) {
    ...
    ASSERT(tiered_add_tenant(mgr, "growing", GV_TIER_SHARED) == 0, "add");
    ASSERT(tiered_promote(mgr, "growing", GV_TIER_DEDICATED) == 0, "promote");

    GV_TenantInfo info;
    memset(&info, 0, sizeof(info));
    ASSERT(tiered_get_info(mgr, "growing", &info) == 0, "get_info");
    ASSERT(info.tier == GV_TIER_DEDICATED, "tier should be DEDICATED after promotion");

    ASSERT(tiered_promote(mgr, "growing", GV_TIER_PREMIUM) == 0, "promote to premium");
    ASSERT(tiered_get_info(mgr, "growing", &info) == 0, "get_info");
...
static int test_record_usage_and_auto_promote(void) {
    ...
    GV_TieredManager *mgr = tiered_create(&cfg);
    ASSERT(mgr != NULL, "create");

    ASSERT(tiered_add_tenant(mgr, "burst", GV_TIER_SHARED) == 0, "add");
    ASSERT(tiered_record_usage(mgr, "burst", 10, 4096) == 0, "record usage");

    int promoted = tiered_check_promote(mgr);
    ASSERT(promoted >= 0, "check_promote should not error");

    GV_TenantInfo info;
    memset(&info, 0, sizeof(info));
    ASSERT(tiered_get_info(mgr, "burst", &info) == 0, "get_info");
...
static int test_list_tenants(void) {
    ...
    ASSERT(tiered_add_tenant(mgr, "s1", GV_TIER_SHARED) == 0, "add s1");
    ASSERT(tiered_add_tenant(mgr, "s2", GV_TIER_SHARED) == 0, "add s2");
    ASSERT(tiered_add_tenant(mgr, "d1", GV_TIER_DEDICATED) == 0, "add d1");

    ASSERT(tiered_tenant_count(mgr) == 3, "total count should be 3");

    GV_TenantInfo shared_list[10];
    int shared_count = tiered_list_tenants(mgr, GV_TIER_SHARED, shared_list, 10);
    ASSERT(shared_count == 2, "should have 2 shared tenants");

    GV_TenantInfo ded_list[10];
    int ded_count = tiered_list_tenants(mgr, GV_TIER_DEDICATED, ded_list, 10);
...
static int test_save_load(void) {
    ...
    GV_TieredManager *mgr = tiered_create(NULL);
    ASSERT(mgr != NULL, "create");
    ASSERT(tiered_add_tenant(mgr, "persist_t", GV_TIER_DEDICATED) == 0, "add");
    ASSERT(tiered_save(mgr, path) == 0, "save");
    tiered_destroy(mgr);

    GV_TieredManager *loaded = tiered_load(path);
    ASSERT(loaded != NULL, "load should succeed");
    ASSERT(tiered_tenant_count(loaded) == 1, "loaded count should be 1");

    GV_TenantInfo info;
    memset(&info, 0, sizeof(info));
    ASSERT(tiered_get_info(loaded, "persist_t", &info) == 0, "get_info after load");
    ASSERT(info.tier == GV_TIER_DEDICATED, "tier should be DEDICATED after load");

    tiered_destroy(loaded);
    remove(path);
    return 0;
}
